        ...
        private void fetchUserData() {
            FirebaseHelper.getInstance().fetchAllUsersFromFirebase(new FirebaseHelper.FirebaseUserCallback() {
                ...
                public void onSuccess(List<Users> users) {
                    ...
                    // 로그 추가: Firebase에서 가져온 사용자 데이터를 출력
                    if (!userList.isEmpty()) {
                        Users firstUser = userList.get(1);
                        Log.d("TestActivity", "First user: " + firstUser.getEmail()); // 첫 번째 사용자 이메일 출력

                        Intent intent = new Intent(Test.this, InventoryProductPage.class);
                        intent.putExtra("selectedUser", firstUser); // Users 객체 전달
                        startActivity(intent);
                    } else {
                        Log.d("TestActivity", "No users found in Firebase.");
                        Toast.makeText(Test.this, "No users found in Firebase.", Toast.LENGTH_SHORT).show();
                    }
                }

                @Override
                public void onFailure(Exception e) {
                    Log.e("TestActivity", "Failed to load users: " + e.getMessage());
                    Toast.makeText(Test.this, "Failed to load users: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        }
    }